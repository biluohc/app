...
fn inner() {
    ...
    fun("/path0 -p 8080,8000,80  /path1 -k /path2 --user Loli,16,./ -V r",
        Err(AppError::Version),
        Fht2p::default());
    fun("/path0 -p 8080,8000,80  /path1 -k /path2 --user Loli,16,./ r -V",
    ...
    fun("src/ -p 8080,8000,80  tests/ -k examples/ --user Loli,16,./ .git run -home $HOME -h",
        ...
        Fht2p::default());
    fun("/path0 -p 8080,8000,80  /path1 -k /path2 --user Loli,16,./ b -h",
        Err(AppError::Help(Some("build".to_owned()))),
        Fht2p::default());

    fun("src -p 8080,8000,80 examples -k tests --user Loli,16,./",
        Ok(()),
        Fht2p::new(vec![8080, 8000, 80u32],
                   true,
                   vec!["src", "examples", "tests"],
                   User::new("Loli", 16, "./"),
                   Run::default(),
                   Build::default()));
    fun("src -p 8080,8000,80 examples -k tests",
        Ok(()),
        Fht2p::new(vec![8080, 8000, 80u32],
                   true,
                   vec!["src", "examples", "tests"],
                   ...
                   Run::default(),
                   Build::default()));
    fun("src -p 8080,8000,80 examples -k tests --user Loli,16,./ r --home $HOME",
        Ok(()),
        Fht2p::new(vec![8080, 8000, 80u32],
                   true,
                   vec!["src", "examples", "tests"],
